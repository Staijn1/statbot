...
test('should add user to database', async () => {
    const userToAdd = new UserPOJO('username', 'id', 0, DateTime.local().toISO(), true, 0, 0, []);
...
test('should update existing user in database', async () => {
    const userToUpdate = new UserPOJO('Eliii', '363384101794873354', 10, '2021-01-14T01:11:07.207+01:00', true, 0, 0, []);
...
test('should do nothing when updating non existing user', async () => {
    const userToUpdate = new UserPOJO('Eliii', 'sdf', 10, '2021-01-14T01:11:07.207+01:00', true, 0, 0, []);
